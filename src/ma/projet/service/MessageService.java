...
public class MessageService implements IDao<Message>{
	...
	public Message getByEmRec(int idEm,int idRec) {
		Message msg=null;
		try {
			String req="select * from message where empEmetteur= ? and empRecepteur= ? ";
			PreparedStatement ps=Connexion.getConnexion().prepareStatement(req);
			ps.setInt(1, idEm);
			ps.setInt(2, idRec);
			ResultSet rs=ps.executeQuery();
			while(rs.next()) {
				msg = new Message(rs.getString("objet"), rs.getString("sujet"),rs.getDate("date"),es.getById(rs.getInt("empEmetteur")),es.getById(rs.getInt("empRecepteur")));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			Logger.getLogger(EmployeService.class.getName()).log(Level.SEVERE, null, e);
	...
	public List<Message> getAll() {
		...
		try {
			...
			ResultSet rs=ps.executeQuery();
			while(rs.next()) {
				messages.add(new Message( rs.getString("objet"), rs.getString("sujet"),rs.getDate("date"),es.getById(rs.getInt("empEmetteur")),es.getById(rs.getInt("empRecepteur"))));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			Logger.getLogger(EmployeService.class.getName()).log(Level.SEVERE, null, e);
